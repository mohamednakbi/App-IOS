...
public struct DateTimePickerDialog: View {
    ...
    private var datePicker: some View {
        let datePickerVar = DatePicker(selection: internalSelection.projectedValue,
                                       in: range,
                                       displayedComponents: components) {
            EmptyView()
        }.labelsHidden()

        var view = AnyView(datePickerVar.datePickerStyle(DefaultDatePickerStyle()))
        if style == .wheel {
            view = AnyView(datePickerVar.datePickerStyle(WheelDatePickerStyle()))
        } else if #available(iOS 14.0, *) {
            if style == .compact {
                view = AnyView(datePickerVar.datePickerStyle(CompactDatePickerStyle()))
            } else if style == .graphical {
                view = AnyView(datePickerVar.datePickerStyle(GraphicalDatePickerStyle()))
            }
        }
...
public extension View {
    func dateTimePickerDialog(isShowing: Binding<Bool>,
                              ...
                              displayComponents components: DatePickerComponents = [
                                  .hourAndMinute,
                                  .date
                              ],
                              ...
                              buttons: [DateTimePickerDialog.Button] = DateTimePickerDialog
                                  .defaultButtons,
                              label: (() -> Text)? = nil) -> some View
    {
        let dialog = DateTimePickerDialog(isShowing: isShowing,
                                          ...
                                          buttons: buttons,
                                          label: label)
        return genericDialog(isShowing: isShowing,
...
//                               foregroundColor: textColor,
                               buttons: [
                                   ...
                                   .cancel(label: "Cancel"),
                                   .custom(label: { Text("Call mom").foregroundColor(.red) },
                                           action: { _ in
